...
//
// The second constructor argument after the name is the ATTACK ROLL multiplier, not a hit chance: the wiki's "attack
// roll modifier" of 100% doubles the roll, which is worth much less than double the accuracy once the roll is already
// well clear of the target's defence. CombatCalc feeds it through attackRoll() for that reason rather than scaling the
// finished chance.
enum SpecialAttack
{
	// Melee
	...
	// Each claw hit is half the one before, with the last two equal. The claws carry no attack roll modifier but roll
	// accuracy up to four times, which specAverageHit models rather than this table
	// - see cascadingAccuracy.
	DRAGON_CLAWS(ItemID.DRAGON_CLAWS, "Slice and Dice", 1, 1, 1.0, 0.5, 0.25, 0.25),
	// Burning claws, whose gameval name is BONE_CLAWS - matched on the id (29577), which is why this was missing for so
	// long. Burning barrage is three hits with a cascade of its own: the maxima below are the first outcome's 25-25-50
	// split of a total reaching 175% of the max hit. Unlike the dragon claws each outcome has its own damage RANGE rather
	// than just a ceiling, so CombatCalc models it separately again.
	...
	// Disrupt is a guaranteed hit rolling between 50% and 150% of the normal max, so it expects a FULL max hit rather than
	// half of the 1.5 below.
	...
	// Eviscerate carries no damage multiplier: it lifts the fang's own narrowed roll back to the true max rather than
	// raising the max, so it is a multiple of the figure before the passive rather than after it, and CombatCalc has it.
	// Its other half is the 50% attack roll modifier, which is here.
	OSMUMTENS_FANG(new int[]{ItemID.OSMUMTENS_FANG, ItemID.OSMUMTENS_FANG_ORNAMENT}, "Eviscerate", 3, 2),

	// Ranged Dark bow figures are for dragon arrows; other arrows spec for 1.3x.
	...
	// Magic These three carry no damage multipliers: their spec damage is not a multiple of the normal max hit, so
	// CombatCalc computes it. The bludgeon scales with missing prayer points and the volatile staff off the magic level.
	...
	// The Dawnbringer, whose gameval name is the room it belongs to rather than the staff - matched on the id, not the
	// name. Pulsate is a flat 75-150 that cannot miss: wiki, "magic damage gear bonuses, prayer and base magic
	// level/boosts do not affect the damage", and Mod Ash confirms there is no accuracy roll either. It is also the one
	// hit Verzik's first phase does not cap, which is why ignoresDamageCap exists.
	DAWNBRINGER(ItemID.VERZIK_SPECIAL_WEAPON, "Pulsate", 1, 1),

	// ---------------------------------------------------------------------
	// Added 2026-08-26, after an audit against the wiki's Special attacks page found the table held 21 of the 73 specials
	// that modify a roll. Every id below was resolved from the wiki's item id and looked up by NUMBER in the gameval dump:
	// a good half of these names are not what they look like
	// - the dragon sword is DRAGON_SHORTSWORD, the armadyl crossbow is ACB, the dogsword is
	// ECHO_GODSWORD, the sunlight spear is WEAPON_OF_SOL, Seercull is DAGANOTH_CAVE_MAGIC_SHORTBOW and the blue moon spear
	// is FROSTMOON_SPEAR.
	// ---------------------------------------------------------------------
	...
	// The melee hit only; Saradomin's Lightning also splashes for magic damage that is not a multiple of this max hit and
	// is not counted.
	...
	// "Roll (up to max roll -5) + 5", so the max is unchanged and the floor is a flat five rather than a share - see
	// flatMinimum.
	GRANITE_HAMMER(ItemID.GRANITE_HAMMER, "Hammer Blow", 3, 2, 1.0),
	// Quick Smash modifies neither roll; it is an instant attack. Here so that a spec still books as a spec rather than
	// falling through to the ordinary hit.
	...
	// Behead scales its attack roll 12-60% and its minimum hit 6-30% with the souls stacked on the axe. The stack is not
	// readable, so both are modelled at the BOTTOM of their range - understating a stacked axe rather than overstating an
	// empty one.
	SOULREAPER_AXE(ItemID.SOULREAPER, "Behead", 28, 25, 1.0),
	SUNLIGHT_SPEAR(ItemID.WEAPON_OF_SOL, "Sol Slam", 1, 1, 1.0),
	// Seeking Lunge also raises the attack roll 70%, but only while the target is below a health threshold, so only the
	// damage half is counted here.
	SUNSPEAR(ItemID.SUNSPEAR, "Seeking Lunge", 1, 1, 1.70),
	// Virulence's floor is the damage the poison it cures would have dealt, which is not readable, so only the attack it
	// makes is counted.
	NOXIOUS_HALBERD(ItemID.NOXIOUS_HALBERD, "Virulence", 1, 1, 1.0),
	DINHS_BULWARK(ItemID.DINHS_BULWARK, "Shield Bash", 6, 5, 1.0),
	// Four accuracy rolls, and unlike the claws the damage depends on how many SUCCEEDED rather than which was first - a
	// binomial, not a cascade.
	CRIMSON_KISTEN(ItemID.CRIMSON_KISTEN, "Brutal Swing", 1, 1, 1.70),
	// Break Shackles scales with the target's remaining bind ticks, which is not readable, so it is scored as an ordinary
	// attack that happens to be a spec.
	BLUE_MOON_SPEAR(ItemID.FROSTMOON_SPEAR, "Break Shackles", 1, 1, 1.0),
	// Bear Down's extra 20 damage arrives over six seconds as its own hitsplats, so it is left out for the same reason
	// burning claws' burn is.
	...
	// Powershot and Soulshot cannot miss, but both roll damage IGNORING void and prayer, which this max hit already
	// includes. Scored at the ordinary max, so they read a little high wherever those are up.
	...
	// Both roll their damage off the wielder's Defence rather than an attack style, and both are flat
	// - see fixedMax.
	...
	// Invocate scales off the magic level like the volatile staff, and its own max is worked out in CombatCalc for the
	// same reason.
	...
	// The attack roll modifier as a RATIONAL rather than a double. The game's own arithmetic is integer, so the roll is
	// scaled as roll * n / d against the finished roll, which is exact where a double multiply can land a point either
	// side of it after truncation. Same lesson as fangMaxHit, which is written max - max * 15 / 100 for the same reason.
	...
	double minFraction()
	{
		switch (this)
		{
			...
			case DAWNBRINGER:
				return 0.5;
			// Behead's floor at an empty axe; it reaches 30% on a full stack, which is not readable.
			case SOULREAPER_AXE:
				return 0.06;
			// Hamstring and Feint both roll from below their normal max: -50% to +50% and -80% to +20% of it.
